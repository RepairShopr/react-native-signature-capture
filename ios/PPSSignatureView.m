...
- (UIImage *)trimmedSignatureImage: (BOOL) rotatedImage withSquare:(BOOL) square
{
    if (!self.hasSignature)
    return nil;
    ...
    if ( UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ) {
        //signature
        if (square) {
            signatureImg = [self cropImage:snapshot];
        }
        else {
            signatureImg = snapshot;
        }
        ...
        if (rotatedImage) {
            if (square) {
                UIImage *rotatedImg = [self rotateImage:snapshot clockwise:false];
                signatureImg =[self cropImage:rotatedImg];
            }
            else {
                UIImage *rotatedImg = [self rotateImage:snapshot clockwise:false];
                signatureImg = [self cropImage:rotatedImg];
...
- (UIImage *)cropImage:(UIImage *)image {
    ...
    CGRect newRect = CGRectMake(left, top, right-left, bottom-top);
    CGImageRef imageRef = CGImageCreateWithImageInRect(image.CGImage, newRect);
    UIImage *newImage = [UIImage imageWithCGImage:imageRef];
    CGImageRelease(imageRef);
    
    if (((right - left) < MIN_WIDTH) && ((bottom - top) < MIN_HEIGHT)) {
        CGSize size = CGSizeMake((MIN_WIDTH - top + left) / 2,(MIN_HEIGHT - bottom + right) / 2);
        return [self imageWithImage:newImage scaledToFillSize:size];
    }
